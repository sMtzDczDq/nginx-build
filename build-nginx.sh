...
PCRE_TAR=$(
	curl --verbose -H "Accept: application/vnd.github+json" "$GH_API_HEADER" --url https://api.github.com/repos/"$REPO_PCRE"/releases/latest --output - | jq '.assets[]|select(.content_type == "application/gzip")|.browser_download_url'
)
